...
class AnchorTabularExplainer(object):
    ...
    def __init__(self, class_names, feature_names, data=None, encoder=None, discretizer=None,
        ...
        if categorical_names:
            ...
            cat_names = sorted(categorical_names.keys())
            self.categorical_features = cat_names
        if discretizer:
            ...
            self.disc = lime.lime_tabular.QuartileDiscretizer(data,
                                                        self.categorical_features,
                                                         feature_names)
    ...
    def get_sample_fn(self, data_row, classifier_fn, desired_label=None):
        def predict_fn(x):
            return classifier_fn(self.encoder(x))
    ...
    def explain_instance(self, data_row, classifier_fn, threshold=0.95,
        ...
        exp['prediction'] = classifier_fn(self.encoder(data_row.reshape(1, -1)))[0]
