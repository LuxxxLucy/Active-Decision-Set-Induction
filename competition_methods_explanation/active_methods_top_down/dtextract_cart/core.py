...
def sampling_new_instance_from_distribution(rule,sampler,domain,population_size=2000,using_trunc=False):
    def exp_normalize(x):
        # b = x.max()
        # y = np.exp(x - b)
        # return y / y.sum()
    ...
    # new_instances = uniform_sampling(tmp_rule,population_size=population_size)
    samples = np.zeros((0,len(domain.attributes)))
    while samples.shape[0] < population_size:
        if using_trunc == True:
            tmp_samples = sampler(population_size*100)
            accepted_samples_indices = np.nonzero(rule.evaluate_data(tmp_samples))
            samples = np.concatenate((samples, tmp_samples[accepted_samples_indices] ), axis=0)
        else:
            pool_of_new_instances = uniform_sampling(tmp_rule,population_size=population_size*100)
            scores = sampler.score_samples(pool_of_new_instances)
            scores = exp_normalize(scores)
            tmp_samples_indices = np.random.choice(np.arange(pool_of_new_instances.shape[0]),size=population_size,p=scores,replace=False)
            samples = np.concatenate((samples, pool_of_new_instances[tmp_samples_indices] ), axis=0)
    return samples[:population_size]

def sampling_new_instance_uniform(rule,domain,population_size=200):
    tmp_rule = extend_rule(rule,domain)
    new_instances = uniform_sampling(tmp_rule,population_size=population_size)
