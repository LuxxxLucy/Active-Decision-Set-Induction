...
def rules_convert(dtextract,data_table,target_class):
    import operator
    import re
    import math
    ...
    this_rule_target_class = 'yes'

    def gather_all_DFS(node,result = None,conditions=None):
        ...
        branch = node.branch
        ind = branch.ind
        threshold = branch.thresh


        conditions_copy = deepcopy(conditions)
        if node.left.__class__ != DTNode:
            rule = Rule(conditions=deepcopy(conditions),domain=data_table.domain,target_class=this_rule_target_class)
            result.append( rule )
        else:
            ...
            if len(conditions) == 0 :
                ...
                used_conditions = [ c.column for c in conditions]
            if ind not in used_conditions:
                if data_table.domain.attributes[ind].is_continuous:
                    condition = Condition(column=ind,max_value=threshold,type='continuous',possible_range=possible_range[ind])
                else:
                    print("categorical not implemented")
            ...
            result = gather_all_DFS(node.left,result,conditions=conditions_copy)

        if node.right.__class__ != DTNode:
            rule = Rule(conditions=deepcopy(conditions),domain=data_table.domain,target_class=this_rule_target_class)
            result.append( rule )
        else:
            # construct new condition
            ...
            if ind not in used_conditions:
                if data_table.domain.attributes[ind].is_continuous:
                    condition = Condition(column=ind,min_value=threshold,type='continuous',possible_range=possible_range[ind])
                else:
                    print("categorical not implemented")
                conditions_copy.append(condition)
            else:
            ...
            result = gather_all_DFS(node.right,result,conditions=conditions_copy)

        return result

    node_list = gather_all_DFS(dtextract.root,result=None)
