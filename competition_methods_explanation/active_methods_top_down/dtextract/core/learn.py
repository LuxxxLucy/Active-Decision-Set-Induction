...
def learnDT(learnDTNode, func, dist, initCons, params):
    ...
    worklist.put(TupleSortingAux((-gain, dtInternalData, dtLeafData, index, depth)))
    score = dtLeafScore
    size = 1
    ...
    while True:
        ...
        if worklist.empty():
            break
        (minusGain, dtInternalData, dtLeafData, index, depth) = worklist.get()
        gain = -minusGain
        ...
        if dtInternalData is None:
            ...
            worklist.put(TupleSortingAux((2.0, dtInternalData, dtLeafData, index, depth)))
            if gain < -1.5:
        ...
        try:
            worklist.put(TupleSortingAux((-gainLeft, dtInternalDataLeft, dtLeafDataLeft, 2*index+1, depth+1)))
            worklist.put(TupleSortingAux((-gainRight, dtInternalDataRight, dtLeafDataRight, 2*index+2, depth+1)))
        except:
            print(gainLeft)
            print( type(gainLeft) )
            print(dtInternalDataLeft)
            print(dtLeafDataLeft)
            print(gainRight)
            print(type(gainRight))
            print(dtLeafDataRight)
            print(dtInternalDataRight)
